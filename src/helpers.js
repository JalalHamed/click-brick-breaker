...
export function getAngle(e) {
	let angle = Math.atan2(state.projectile.pos.y - e.y, state.projectile.pos.x - e.x);
...
export function getColorsDifferences(color1, color2) {
	let differences = [];
	for (let i = 0; i < 3; i++)
		differences.push(Math.abs(convertRGBtoArr(color1)[i] - convertRGBtoArr(color2)[i]));
	return differences;
}
...
export function getPointerYPos(basedOn, { gradient, YAxisIntercept, angle }) {
	...
	if (angle > MIN_ANGLE && angle < MAX_ANGLE) {
		...
		if (y < topBorder.heightFromTop + radius) return topBorder.heightFromTop + radius;
	}
	...
	if (angle === MAX_ANGLE)
		return (
			bottomBorder.pos.y -
			bottomBorder.height -
			Math.tan(Math.PI - angle) * (CANVAS.width - state.projectile.pos.x + radius)
		);
}
